...
// =================================================================
// Validation Breast Cancer Wisconsin (UCI, via sklearn) -- meme
// methodologie que Iris/Digits/XOR : boosting AdaBoost avec routeur
// PONDERE reconstruit a chaque round (tsetlin_engine.h), combineur
// par argmax (ici binaire) sur la somme ponderee (alpha) des scores
// normalises, et baseline "routeurs ponderes seuls" (purete, sans
// clause) pour verifier que les clauses apportent bien quelque
// chose.
//
// 569 exemples, 30 features continues -> 120 bits (thermometre,
// 4 bits/feature, seuils = quartiles empiriques). 2 classes
// (malin/benin). Split 80/20, reshuffle a chaque run (comme Iris).
// =================================================================
...
int main(int argc, char** argv) {
    ...
    const int    K             = argc > 1 ? atoi(argv[1]) : 2;
    const int    N             = 320;
    ...
    const int    M             = 16;
    const int    nbRuns        = argc > 2 ? atoi(argv[2]) : 40;

    auto allRaw = loadAll("data/BinaryBreastCancerData.txt", n);
    const int trainSize = (int)std::round(0.8 * allRaw.size());

    cout << fixed << setprecision(1);
    cout << "Dataset : " << allRaw.size() << " exemples  train=" << trainSize
