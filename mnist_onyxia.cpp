...
// =================================================================
// Fichier UNIQUE et autonome (moteur minimal + test MNIST) pour
// lancement sur Onyxia sans dependance au reste du projet.
// =================================================================
...
struct Params { double S1, S2, S3, S4, a1, a2; };
...
struct LabeledExample { vector<int> x; int y; };

// =================================================================
// Methode validee : stumps aleatoires (K features fixees), notre
// regle a 4 cas (updateMasked), sans routeur, chaque clause = un
// round AdaBoost independant. Parallelise par classe (thread_local rng).
// =================================================================
...
int main(int argc, char** argv) {
    ...
    int N = 256, total = 20000, M = 300, nbRuns = 1, K = 1;
    double S = 1, a = 0.3;

    M = argc > 1 ? atoi(argv[1]) : 300;
    total = argc > 2 ? atoi(argv[2]) : 20000;
    ...
    // Chemins : donnees venant de git clone https://github.com/cair/fast-tsetlin-machine-with-mnist-demo.git
    // puis unzip BinarizedMNISTData.zip (fichiers MNISTTraining.txt / MNISTTest.txt)
    auto trainRawAll = loadAll("fast-tsetlin-machine-with-mnist-demo/MNISTTraining.txt", n);
    auto testRawAll  = loadAll("fast-tsetlin-machine-with-mnist-demo/MNISTTest.txt", n);
