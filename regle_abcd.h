...
// ---------------------------------------------------------------
// Paramètres de la règle d'apprentissage à 4 cas (Definition 1,
// preuve_complete.tex). L'ancienne règle (S,alpha) est le cas particulier
// a=c=S, b=alpha, d=0.
// ---------------------------------------------------------------
struct Params {
    double a, b, c, d;
...
struct Clause {
    ...
    // Règle d'apprentissage à 4 cas (Definition 1, preuve_complete.tex),
    // appliquée symétriquement à v[i] (littéral x_i) et vbar[i] (littéral ¬x_i) :
    //
    //   x_i | y | v[i] | vbar[i]
    //   ----+---+------+--------
    //    0  | 0 |  +a  |   -c
    ...
    void update(const vector<int>& x, int y, const Params& p) {
        ...
        for (int i = 0; i < n; i++) {
            if (x[i] == 0) {
                if (y == 0) { v[i].towardInclude(a); vbar[i].towardExclude(c); }
                else        { v[i].towardExclude(b); vbar[i].towardInclude(d); }
            } else {
                if (y == 0) { v[i].towardExclude(c); vbar[i].towardInclude(a); }
                else        { v[i].towardInclude(d); vbar[i].towardExclude(b); }
            }
        }
    }
