...
// =================================================================
// Monk-1/2/3 (UCI, benchmark tiers reconnu pour l'apprentissage de
// concepts booleens combinatoires) -- meme methodologie que
// XOR/Iris/Digits : boosting AdaBoost, routeur PONDERE reconstruit a
// chaque round, combineur par somme ponderee des scores normalises,
// baseline "routeurs ponderes seuls" (purete, sans clause).
//
// 6 attributs categoriels (cardinalites [3,3,2,3,4,2]) -> 17 bits
// one-hot. Splits train/test OFFICIELS et FIXES (comme fournis par
// UCI), pas de reshuffle -- meme convention que XOR/Digits.
//
// Monk-2 partage une propriete structurelle avec XOR : le concept
// ("exactement 2 des 6 attributs ont leur 1ere valeur") n'est pas
// separable par des splits axe-alignes successifs sans lookahead --
// on attend donc un avantage pour notre methode, coherent avec les
// resultats XOR (pas necessairement du meme ordre de grandeur).
//
// Usage : ./validation_monk <1|2|3> [K] [nbRuns]
// =================================================================
...
int main(int argc, char** argv) {
    if (argc < 2) { cerr << "Usage: " << argv[0] << " <1|2|3> [K] [nbRuns]\n"; return 1; }
    const int monkNum = atoi(argv[1]);

    const int    n             = 17;   // 6 attributs one-hot : [3,3,2,3,4,2]
    const int    K             = argc > 2 ? atoi(argv[2]) : 2;
    const int    N             = 320;
    const Params p             = {1,1,1,1,0.3,0.3};
    const int    total         = 30000;
    const int    discoverCount = max(300, 3 * n);
    const double noiseTol      = (monkNum == 3) ? 0.10 : 0.02; // Monk-3 a ~5% de bruit connu sur le train
    const int    M             = 10;
    const int    nbRuns        = argc > 3 ? atoi(argv[3]) : 20;

    string trainPath = "data/Monk" + to_string(monkNum) + "TrainingData.txt";
    string testPath  = "data/Monk" + to_string(monkNum) + "TestData.txt";
    auto trainData = loadDataset(trainPath, n);
    auto testData  = loadDataset(testPath,  n);

    cout << fixed << setprecision(1);
    cout << "Monk-" << monkNum << " : train=" << trainData.size() << " test=" << testData.size()
         << "  K=" << K << " M=" << M << " (" << nbRuns << " runs)\n\n";
