...
int main(int argc, char** argv) {
    const int muxSize = argc > 1 ? atoi(argv[1]) : 6;   // 6 ou 11
    const int n = muxSize;                              // nombre de features = taille du multiplexeur
    ...
    const double noiseTol = 0.02;  // dataset propre, sans bruit
    const int M = 5;
    ...
    cout << "Dataset : Mux" << muxSize << " train=" << trainData.size() << " test=" << testData.size()
         << "  K=" << K << " M=" << M << " (" << nbRuns << " runs)\n\n";
